...
.*Réponse :* +
+
_Bibliothèque 1_ :  `stuctpot` : Parser des arguments pris en ligne de commande (documentation : https://docs.rs/structopt/0.3.25/structopt/). +
_Bibliothèque 2_ : `walkdir` : Pour le parcour recursif des répertoires (documentation : https://docs.rs/walkdir/2.3.2/walkdir/). +
...
+
_Modules_ : +
`cli`: Permet l'analyse des arguments pris en ligne de commande. +
`scan` : Durant la recherche de fichiers, permet le scan recursif d'un dossier. +
`musicfile` : Strucutre de stockaage des metadonnées d'un fichier musical. +
+
_Fichiers_ : +
`lib.rs` : Contient la bibliothèque du projet. +
`main.rs` : Contient l'exécutable du projet. + 
